...
WHERE track_id IN (SELECT track_id FROM track WHERE genre_id IN (SELECT genre_id
														  FROM Genre
														  WHERE name = 'Rock')
)
ORDER BY c.email
...
-- Q11: We would like to find out the most popular music genre for each country.
...
SELECT * FROM the_most_popular_genre WHERE r <= 1

-- Q12: Write a query that retrieves the customer spending the most on music for each country and returns the country, the top customer, the genre, and the amount they spent.
